...
namespace Essentia
{
    ...
    void LightSource::SetColor(const glm::vec3& col) { color = col; }
    ...
    void LightSource::SetAmbient(const glm::vec3& amb) { ambient = amb; }

    const glm::vec3& LightSource::GetDiffuse() const { return diffuse; }
    void LightSource::SetDiffuse(const glm::vec3& diff) { diffuse = diff; }

    const glm::vec3& LightSource::GetSpecular() const { return specular; }
    void LightSource::SetSpecular(const glm::vec3& spec) { specular = spec; }

    float LightSource::GetIntensity() const { return intensity; }
    void LightSource::SetIntensity(float value) { intensity = value; }

    LightType LightSource::GetType() const { return type; }

    float LightSource::GetInnerCutOff() const { return innerCutOff; }
    void LightSource::SetInnerCutOff(float cutOff) { innerCutOff = cutOff; }

    float LightSource::GetOuterCutOff() const { return outerCutOff; }
    void LightSource::SetOuterCutOff(float cutOff) { outerCutOff = cutOff; }

    float LightSource::GetConstant() const { return constant; }
    void LightSource::SetConstant(float value) { constant = value; }

    float LightSource::GetLinear() const { return linear; }
    void LightSource::SetLinear(float value) { linear = value; }

    float LightSource::GetQuadratic() const { return quadratic; }
    void LightSource::SetQuadratic(float value) { quadratic = value; }
